...
func TestFindWithComparer(t *testing.T) {
	testWrap(t, func(bh *badgerhold.Store, t *testing.T) {
		store := generichold.Open[CItemTest](bh)
		...
		result, err := store.Find(badgerhold.Where("Inner").Gt(data[1].Inner))
		if err != nil {
	...
	testWrap(t, func(bh *badgerhold.Store, t *testing.T) {
		...
		for i := 0; i < to.NumField(); i++ {
			...
			t.Run(fmt.Sprintf("Builtin type %s equal", to.Field(i).Name), func(t *testing.T) {
				...
				// equal
				result, err := store.Find(badgerhold.Where(to.Field(i).Name).Eq(curField))
				if err != nil {
			...
			t.Run(fmt.Sprintf("Builtin type %s greater than", to.Field(i).Name), func(t *testing.T) {
				// gt
				result, err := store.Find(badgerhold.Where(to.Field(i).Name).Gt(curField))
				if err != nil {
					t.Fatalf("Error finding equal result %s", err)
				}
			...
			t.Run(fmt.Sprintf("Builtin type %s less than", to.Field(i).Name), func(t *testing.T) {
				// lt
				result, err := store.Find(badgerhold.Where(to.Field(i).Name).Lt(curField))
				if err != nil {
					t.Fatalf("Error finding equal result %s", err)
				}
