...
// completed.
func benchWrap(b *testing.B, options *badgerhold.Options, bench func(bh *badgerhold.Store, b *testing.B)) {
...
func BenchmarkRawInsert(b *testing.B) {
	...
	benchWrap(b, nil, func(bh *badgerhold.Store, b *testing.B) {
		store := generichold.Open[BenchData](bh)
...
func BenchmarkIndexedInsert(b *testing.B) {
	benchWrap(b, nil, func(bh *badgerhold.Store, b *testing.B) {
		store := generichold.Open[BenchDataIndexed](bh)
...
func BenchmarkNoIndexInsertJSON(b *testing.B) {
	...
	benchWrap(b, &opt, func(bh *badgerhold.Store, b *testing.B) {
		store := generichold.Open[BenchData](bh)
...
func BenchmarkFindNoIndex(b *testing.B) {
	benchWrap(b, nil, func(bh *badgerhold.Store, b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			_, err := store.Find(badgerhold.Where("Category").Eq("findCategory"))
			if err != nil {
